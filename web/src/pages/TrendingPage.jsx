/**
 * TrendingPage — NovaSound TITAN LUX v333
 * Page des tendances : sons et artistes en vogue par période.
 * FIXES v333 :
 *  - Traduction complète en français (titre, badges, labels)
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col pb-36 md:pb-28">
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
          <div className="flex gap-2 bg-gray-900/60 border border-white/[0.08] rounded-2xl p-1">
            {PERIODS.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${period === id ? 'bg-gradient-to-r from-cyan-500 to-fuchsia-500 text-white shadow-md' : 'text-gray-500 hover:text-white'}`}
              >
          ...
          <div className="flex gap-1 bg-gray-900/60 border border-white/[0.08] rounded-2xl p-1">
            <button
            ...
            >
              {loadingSongs ? (
                ...
                <div className="text-center py-20 text-gray-600">
                  ...
                  <p className="text-sm mt-1 text-gray-700">{'Reviens bientôt !'}</p>
                </div>
              ) : (
                <>
                  {/* Top 3 heroes */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    {songs.slice(0, 3).map((song, idx) => (
                      <motion.div
                        ...
                        className="relative bg-gray-900/70 border border-white/10 rounded-2xl overflow-hidden group cursor-pointer hover:border-cyan-500/30 transition-all active:scale-[0.98]"
                      ...
                      >
                        ...
                        <div className="relative p-4">
                          <div className="flex items-start gap-3">
                            ...
                            <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 shadow-lg">
                              {song.cover_url
                                ...
                                : <div className="w-full h-full bg-gradient-to-br from-cyan-500 to-fuchsia-500 flex items-center justify-center"><Music className="w-6 h-6 text-white" /></div>
                              }
                            </div>
                          ...
                          <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="w-8 h-8 bg-gradient-to-r from-cyan-500 to-fuchsia-500 rounded-full flex items-center justify-center shadow-lg">
                  ...
                  <div className="space-y-1.5">
                    {songs.slice(3).map((song, idx) => (
                      <motion.div
                        ...
                        className="flex items-center gap-3 px-4 py-3 rounded-2xl hover:bg-white/[0.04] transition-all group cursor-pointer border border-transparent hover:border-white/[0.08] active:scale-[0.99]"
                        onClick={() => playSong(song, songs)}
                      >
                  ...
                  {songs.length > 0 && (
                    <div className="mt-6 flex justify-center">
                      <button
                        ...
                        className="flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-fuchsia-500 hover:from-cyan-400 hover:to-fuchsia-400 text-white font-bold px-8 py-3 rounded-full transition-all shadow-lg shadow-cyan-500/20 active:scale-95"
                      >
                        <Play className="w-4 h-4 fill-current" />
                        Écouter le Top {songs.length}
                      </button>
                    </div>
                  )}
                </>
            ...
            >
              {loadingArtists ? (
                ...
                <div className="text-center py-20 text-gray-600">
                  ...
                  <p className="font-medium">Aucun artiste en tendance pour cette période</p>
                </div>
              ) : (
                  ...
                  {artists.map((artist, idx) => (
                    ...
                    >
                      <Link to={`/artist/${artist.user_id}`}
                        className="flex items-center gap-4 p-4 bg-gray-900/60 border border-white/[0.08] rounded-2xl hover:border-cyan-500/30 hover:bg-gray-900/90 transition-all group"
                      >
                        <div className="relative flex-shrink-0">
                          <div className="w-12 h-12 rounded-full overflow-hidden shadow-lg">
                            {artist.avatar_url
                              ? <img src={artist.avatar_url} alt="" className="w-full h-full object-cover" />
                              : <div className="w-full h-full bg-gradient-to-br from-cyan-500 to-fuchsia-500 flex items-center justify-center"><Users className="w-6 h-6 text-white" /></div>
                            }
                          </div>
