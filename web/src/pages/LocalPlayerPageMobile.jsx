...
const LocalPlayerPageMobile = memo(() => {
  ...
  const activeSongForFav = currentSong?.is_local ? currentSong : null;
  const favorited = activeSongForFav ? localFavoriteIds.has(activeSongForFav.id) : false;
          ...
          {!activeSong ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-6 px-8 text-center">
              <div className="relative w-32 h-32">
                ...
                {/* Halos pulsants */}
                <motion.div animate={{scale:[1,1.2,1],opacity:[0.15,0.35,0.15]}} transition={{duration:3,repeat:Infinity,ease:'easeInOut'}}
                ...
                <motion.div
                  ...
                  <motion.div animate={{scale:[1,1.08,1]}} transition={{duration:2,repeat:Infinity,ease:'easeInOut'}}>
                    <Music2 className="w-8 h-8 text-white drop-shadow-lg"/>
                  </motion.div>
                </motion.div>
              </div>
              <div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-h-0 px-6 pt-2">
              ...
              <div className="flex-shrink-0 flex items-start justify-between gap-3 mb-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0 mt-1">
                  ...
                  <button onClick={()=>toggleLocalFavorite(activeSongForFav)} className={`p-2 rounded-full transition-all active:scale-90 ${favorited?'text-pink-500':'text-gray-600 hover:text-pink-400'}`}>
      ...
      {activeTab === 'favoris' && (
        <div className="relative z-10 flex-1 flex flex-col overflow-hidden min-h-0">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1">
                <p className="text-white font-black text-lg flex items-center gap-2">
                  <Heart className="w-5 h-5 text-pink-400 fill-current"/>Favoris
                </p>
                <p className="text-gray-500 text-xs">{localFavoriteSongs?.length||0} titre{(localFavoriteSongs?.length||0)!==1?'s':''}</p>
              </div>
              {localFavoriteSongs?.length>0&&(
                <button
                  onClick={()=>{
                    const playable=localFavoriteSongs.filter(s=>!s._needsReimport);
                    const live=playable.map(s=>songs.find(q=>q.id===s.id)).filter(Boolean);
                    if(live.length>0)playSong(live[0],live);
                  }}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold active:scale-90 transition-all"
                  style={{background:'rgba(236,72,153,0.12)',border:'1px solid rgba(236,72,153,0.25)',color:'#f472b6'}}>
                  <Play className="w-4 h-4 fill-current"/>Tout lire
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-2 pb-4" style={{scrollbarWidth:'none'}}>
            {!localFavoriteSongs?.length?(
              <div className="flex flex-col items-center justify-center py-24 text-center px-8">
                <motion.div animate={{scale:[1,1.18,1]}} transition={{duration:2.2,repeat:Infinity,ease:'easeInOut'}}>
                  <Heart className="w-14 h-14 text-pink-500/25 mb-4"/>
                </motion.div>
                <p className="text-gray-500 text-sm leading-relaxed">Aucun favori pour l'instant.<br/>Clique sur ❤ pendant la lecture pour ajouter un son.</p>
              </div>
            ):(
              <div className="space-y-1">
                {localFavoriteSongs.map((song,i)=>{
                  ...
                  const liveRef  = songs.find(s=>s.id===song.id);
                  return(
                    <motion.div key={song.id} layout
                      initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}}
                      transition={{delay:i*0.03}}
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all active:scale-[0.98] ${isActive?'bg-white/[0.08] border border-pink-500/25':'hover:bg-white/[0.05] border border-transparent'}`}
                      onClick={()=>{ if(liveRef) playSong(liveRef, songs.filter(s=>localFavoriteIds.has(s.id))); }}>
                      {/* Cover */}
                      <div className="w-11 h-11 rounded-xl overflow-hidden flex-shrink-0 relative">
                        ...
                        {isActive&&isPlaying&&(
                          <div className="absolute inset-0 bg-black/50 flex items-center justify-center gap-[2px]">
                            {[0,1,2].map(j=>(
                              <motion.div key={j} className="w-1 rounded-full bg-pink-400"
                      ...
                      {/* Info */}
                      <div className="flex-1 min-w-0">
                        <NoTranslate tag="p" className={`text-sm font-semibold truncate ${isActive?'text-pink-400':'text-white'}`}>{song.title}</NoTranslate>
                        <NoTranslate tag="p" className="text-xs text-gray-600 truncate">{song.artist}</NoTranslate>
                        {!liveRef&&<p className="text-[10px] text-amber-500/70 mt-0.5">Réimporter le fichier</p>}
                      </div>
                      {/* Remove */}
                      <button
                        onClick={e=>{e.stopPropagation();toggleLocalFavorite(song);}}
                        className="p-2 rounded-xl text-pink-400 active:scale-75 transition-all flex-shrink-0"
      ...
      <div className="relative z-30 flex-shrink-0 border-t border-white/[0.06]"
        ...
        <div className="flex">
          {[
            ...
            { key:'favoris', icon:Heart,   label:'Favoris' },
          ].map(({key,icon:Icon,label})=>(
            <button key={key} onClick={()=>setActiveTab(key)}
              className={`flex-1 flex flex-col items-center gap-1 py-3 transition-all active:scale-90 ${activeTab===key?(key==='favoris'?'text-pink-400':'text-cyan-400'):'text-gray-600'}`}>
              <div className="relative">
                <Icon className={`w-5 h-5 ${key==='favoris'&&activeTab===key?'fill-current':''}`}/>
                ...
                {/* Badge count on favoris tab */}
                {key==='favoris'&&localFavoriteSongs?.length>0&&(
                  <span className="absolute -top-1.5 -right-2 min-w-[14px] h-3.5 px-0.5 flex items-center justify-center rounded-full bg-pink-500 text-[8px] font-black text-white leading-none">
                    {localFavoriteSongs.length}
                  </span>
                )}
              </div>
