/**
 * ChatPage — NovaSound TITAN LUX v120
 * Chat Public Global
 ...
 * - Réactions emoji · Realtime · Présence
 */
...
import {
  ...
  Loader2, X, Smile, Users, Music, Bell, AtSign,
} from 'lucide-react';
...
// Highlight @mentions dans le texte
...
const ChatPage = () => {
  ...
  const [activeTab,     setActiveTab]     = useState('global'); // 'global' | 'mentions'
  ...
  // ── Gestion lien entrant : ?highlight=msgId&tagger=username ───────
  useEffect(() => {
  ...
  const handleTextChange = useCallback((e) => {
    ...
    // Détecte si on tape @quelquechose
  ...
  // ── Charger mes mentions ───────────────────────────────────────────
  const fetchMyMentions = useCallback(async () => {
    if (!currentUser?.user_metadata?.username && !currentUser?.email) return;
    setLoadingMent(true);
    try {
        ...
        <div className="flex-1 flex flex-col" style={{ height: 'calc(100vh - 64px)' }}>
          ...
          <div className="flex-shrink-0 border-b border-white/[0.06] bg-gray-950/95 backdrop-blur-sm px-4 py-3">
            <div className="max-w-3xl mx-auto">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-[11px] mt-0.5">Communauté NovaSound · tague @username</p>
                  </div>
                </div>
                {onlineCount > 0 && (
              ...
              {/* Onglets : Chat global / Mes messages */}
              <div className="flex items-center gap-2 mb-3">
                ...
                {currentUser && (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all border ${
                      activeTab === 'mentions'
                        ? 'bg-gradient-to-r from-magenta-500 to-pink-600 text-white border-transparent shadow-lg shadow-pink-500/20'
                        : 'bg-white/5 text-gray-500 border-white/[0.07] hover:bg-white/10 hover:text-gray-300'
                    }`}>
              ...
              {/* Filtres période — uniquement pour chat global */}
              {activeTab === 'global' && (
                    ...
                    {myMentions.map(msg => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            setActiveTab('global');
                            // Navigate to chat with highlight + auto-tag the sender
          ...
          {activeTab === 'global' && (
            <>
              ...
              {/* ── Zone de saisie ── */}
              <div className="flex-shrink-0 border-t border-white/[0.06] bg-gray-950/95 backdrop-blur-sm px-4 py-3 pb-safe relative">
                <div className="max-w-3xl mx-auto">
