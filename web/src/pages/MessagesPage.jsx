/**
 * MessagesPage — NovaSound TITAN LUX v70
 * Interface messagerie directe complète
 * - Liste des conversations (sidebar)
 * - Fil de messages en temps réel
 * - Barre de recherche d'utilisateurs pour démarrer une conv
 * - Mobile : plein écran sur la conversation sélectionnée
 */
...
import {
  ...
  MoreVertical, Trash2, Music, CheckCheck, Check
} from 'lucide-react';
...
const MessagesPage = () => {
  ...
  const { conversations, totalUnread, fetchConversations, fetchMessages, sendMessage, markConversationRead, deleteMessage } = useMessages();

  // Conversation sélectionnée
  const [selectedConv, setSelectedConv]   = useState(null); // { other_user_id, other_user }
  const [messages,     setMessages]       = useState([]);
  const [loadingMsgs,  setLoadingMsgs]    = useState(false);
  const [newMsg,       setNewMsg]         = useState('');
  const [sending,      setSending]        = useState(false);

  // Recherche d'utilisateur pour nouvelle conv
  ...
  // Mobile : afficher le fil ou la liste
  const [mobileView,   setMobileView]     = useState('list'); // 'list' | 'chat'

  // Auto-ouvrir une conversation depuis ArtistProfilePage (location.state)
  useEffect(() => {
  ...
  const openConversation = useCallback(async (conv) => {
    ...
    // Souscrire au realtime de cette conversation
  ...
  // Focus input quand conv sélectionnée
  useEffect(() => {
    if (selectedConv) setTimeout(() => inputRef.current?.focus(), 200);
  ...
  const startConvWithUser = (user) => {
    setShowSearch(false);
    setSearchQuery('');
    setSearchResults([]);
    // Créer une conv synthétique si elle n'existe pas encore
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-white/[0.06] flex-shrink-0">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            onClick={() => { setShowSearch(true); setTimeout(() => document.getElementById('user-search')?.focus(), 100); }}
        ...
        <AnimatePresence>
          {showSearch && (
            ...
            >
              ...
              {searchResults.length > 0 && (
                <div className="bg-gray-800/90 border border-white/[0.07] rounded-xl overflow-hidden mb-2 max-h-52 overflow-y-auto">
                  {searchResults.map(user => (
                    <button key={user.id} onClick={() => startConvWithUser(user)}
              ...
              {searching && (
                <p className="text-gray-600 text-xs text-center py-2">Recherche…</p>
              )}
            </motion.div>
          )}
        </AnimatePresence>
          ...
          conversations.map(conv => {
            ...
            return (
              <button key={conv.other_user_id} onClick={() => openConversation(conv)}
                className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-white/[0.04] transition-colors border-b border-white/[0.03] relative ${isActive ? 'bg-cyan-500/[0.07]' : ''}`}>
                ...
                <div className="flex-1 min-w-0 text-left">
                  ...
                  {conv.last_message && (
                    <p className={`text-xs truncate mt-0.5 ${conv.unread_count > 0 ? 'text-gray-300' : 'text-gray-600'}`}>
                      {conv.last_message_sender_id === currentUser.id ? 'Vous : ' : ''}{conv.last_message}
                    </p>
                  )}
                </div>
              </button>
  ...
  const ChatView = () => {
    ...
    const other = selectedConv.other_user || conversations.find(c => c.other_user_id === selectedConv.other_user_id)?.other_user;
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/[0.06] flex-shrink-0 bg-gray-950/80 backdrop-blur-sm">
          <button onClick={() => { setMobileView('list'); setSelectedConv(null); }}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`group relative max-w-[72%] sm:max-w-[60%]`}>
                        <div
                          className={`px-3.5 py-2 text-sm leading-relaxed select-text ${
                            isMine
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-950 flex flex-col pb-24 md:pb-0">
        ...
        <div className="flex-1 flex" style={{ height: 'calc(100vh - 64px - 80px)' }}>
          {/* ── Sidebar conversations (desktop: toujours visible, mobile: conditionnelle) ── */}
          <div className={`
          ...
          {/* ── Zone chat (desktop: toujours visible, mobile: conditionnelle) ── */}
          <div className={`
