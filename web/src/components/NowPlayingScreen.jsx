/**
 * NowPlayingScreen — NovaSound TITAN LUX v6000
 *
 * CORRECTION CRITIQUE :
 * L'ancienne version utilisait createMediaElementSource(audioRef) pour
 * connecter un AnalyserNode Web Audio. Cette API prend le CONTRÔLE EXCLUSIF
 * de l'élément <audio>, le déconnecte de la sortie son, et provoque :
 *   - Silence total dès l'ouverture de l'écran
 *   - Freeze de tous les boutons (l'audio context bloque le thread UI)
 *   - Crash impossible à récupérer sans reload de la page
 *   - Erreur DOMException à chaque réouverture (source déjà connectée)
 *
 * Fix : SUPPRESSION du AnalyserNode + createMediaElementSource.
 * Le visualiseur canvas utilise maintenant une animation CSS idle wave pure
 * (sin wave animée) — aucune interaction avec l'audio pipeline.
 */
...
// ── Idle wave canvas — AUCUNE Web Audio API, aucun lien avec <audio> ────────
const IdleWave = ({ isPlaying, color }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const t = frame / 60;
      const amp = isPlaying ? 10 : 5;

      ctx.beginPath();
      for (let x = 0; x <= W; x += 2) {
        const y = H / 2
          + Math.sin(x / 28 + t * 1.8) * amp
          + Math.sin(x / 55 + t * 0.9) * amp * 0.6;
      ...
      if (isPlaying) {
        ...
        for (let x = 0; x <= W; x += 2) {
          const y = H / 2
            + Math.sin(x / 35 + t * 2.2 + 1.2) * amp * 0.7
            + Math.sin(x / 70 + t * 1.1) * amp * 0.4;
          x === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
        }
...
const NowPlayingScreen = ({
  ...
  const genreColors = {
    ...
    'Drill': '#ef4444', 'Pop': '#06b6d4', 'Zouk': '#f43f5e',
    'Afropop': '#f59e0b', 'Soul': '#ec4899',
  };
  ...
  return (
    ...
    >
      ...
      {/* Idle wave — CSS canvas pur, zero impact audio */}
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-center py-2 min-h-0">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              animate={{
                scale: isPlaying ? [1, 1.025, 1] : 0.96,
                opacity: 1,
              }}
              exit={{ scale: 0.82, opacity: 0 }}
              transition={{
                duration: isPlaying ? 2.5 : 0.4,
                repeat: isPlaying ? Infinity : 0,
                ease: 'easeInOut',
              }}
              className="w-full max-w-[260px] aspect-square rounded-3xl overflow-hidden"
              style={{
                boxShadow: `0 0 80px ${color}45, 0 24px 60px rgba(0,0,0,0.8)`,
              }}
            >
              {currentSong.cover_url
        ...
        <div className="flex items-center gap-4 mb-4 flex-shrink-0">
          <div className="flex-1 min-w-0">
            <AnimatePresence mode="wait">
              <motion.p
                ...
                className="text-white text-xl font-black truncate"
              >
                {currentSong.title}
              </motion.p>
            </AnimatePresence>
            <Link
              to={`/artist/${currentSong.uploader_id}`}
              onClick={onClose}
          ...
          >
            <Heart className={`w-7 h-7 transition-all ${isLiked
              ? 'fill-current text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]'
        ...
        <div className="flex items-center justify-around mb-5 flex-shrink-0">
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 text-xs transition-all active:scale-90 ${
              showLyrics ? 'text-fuchsia-400' :
              lyricsContent ? 'text-gray-400 hover:text-white' :
          ...
          >
            ...
            {repeat === 'one' && (
              <span className="absolute -top-1 -right-1 text-[8px] bg-cyan-500 text-black font-black rounded-full w-3.5 h-3.5 flex items-center justify-center">1</span>
            )}
          </button>
        </div>
                  ...
                  : upcoming.map((s, i) => (
                    <div key={s.id} className="flex items-center gap-3 py-1.5">
                      ...
                      <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-800 flex-shrink-0">
                        {s.cover_url
                          ? <img src={s.cover_url} className="w-full h-full object-cover" alt={s.title} />
