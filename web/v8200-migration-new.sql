...
-- Évolutions majeures + correction sélection MP3 iPhone
-- Date: 2025-03-02
-- ══════════════════════════════════════════════════════════════════════

-- ── 1. Tables de messagerie avancée ───────────────────────────────
...
-- Politiques conversations privées
...
-- Politiques types notifications
...
-- Amélioration table notifications
...
-- Index pour performances notifications
...
BEGIN
  ...
  LEFT JOIN auth.users u ON (
    (pc.user1_id::text = p_user_id::text AND u.id = pc.user2_id) OR
    (pc.user2_id::text = p_user_id::text AND u.id = pc.user1_id)
  )
...
-- Index pour performances conversations
...
SELECT 
  ...
  COUNT(DISTINCT CASE WHEN l.follower_id = u.id THEN l.following_id END) as following_count,
  COUNT(DISTINCT CASE WHEN l.following_id = u.id THEN l.follower_id END) as followers_count,
  COUNT(DISTINCT CASE WHEN lik.song_id IS NOT NULL THEN lik.user_id END) as total_likes,
...
LEFT JOIN follows l ON (l.follower_id::text = u.id::text OR l.following_id::text = u.id::text)
...
-- ── 8. Corrections pour compatibilité iPhone ─────────────────────────────

-- Ajout de colonne pour les métadonnées iOS
ALTER TABLE songs
  ADD COLUMN IF NOT EXISTS ios_metadata JSONB DEFAULT '{}';

-- Ajout de colonne pour le support iOS
ALTER TABLE songs
  ADD COLUMN IF NOT EXISTS is_ios_compatible BOOLEAN NOT NULL DEFAULT true;

-- Index pour les fichiers iOS compatibles
