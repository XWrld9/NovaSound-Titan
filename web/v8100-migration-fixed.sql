-- ══════════════════════════════════════════════════════════════════════════
-- NovaSound TITAN LUX — Migration v8100 (CORRIGÉE)
...
-- 8. Fonction pour compter les messages non lus d'un utilisateur (CORRIGÉ)
CREATE OR REPLACE FUNCTION get_unread_count(p_user_id UUID)
RETURNS INTEGER AS $$
  SELECT COUNT(*)::INTEGER
  FROM messages
  WHERE recipient_id::uuid = p_user_id AND is_read = false;
...
-- 11. Vue pour les statistiques de messages par conversation (CORRIGÉE)
CREATE OR REPLACE VIEW conversation_summary AS
SELECT
  LEAST(sender_id, recipient_id) AS user1_id,
  GREATEST(sender_id, recipient_id) AS user2_id,
  MAX(created_at) AS last_message_at,
  COUNT(*) FILTER (WHERE is_read = false AND recipient_id::uuid = auth.uid()) AS unread_count
FROM messages
GROUP BY 1, 2;
...
CREATE POLICY "playlists_select_public"
  ...
  USING (is_public = true OR auth.uid()::text = owner_id::text);
...
CREATE POLICY "playlist_songs_select"
  ...
  USING (
    EXISTS (
      ...
      AND (p.is_public = true OR auth.uid()::text = p.owner_id::text)
    )
  );
