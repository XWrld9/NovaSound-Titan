/**
 * send-push-notification — NovaSound V28000 (FIXED)
 *
 * FIXES v28000 :
 ...
 * ✅ Fixed result handling and error management
 */
...
Deno.serve(async (req: Request) => {
  ...
  // ✅ FIXED: Better error handling for Promise.allSettled
  ...
  results.forEach((result, index) => {
    if (result.status === "fulfilled") {
      const value = result.value as { ok: boolean; status?: number; endpoint: string };
      if (value.ok) {
        successful.push(index);
      } else if ([404, 410].includes(value.status ?? 0)) {
        expired.push(value.endpoint);
      }
    }
  });
  ...
  if (expired.length) {
    try {
      ...
      console.log(`[Push] Purged ${expired.length} expired subscriptions`);
    } catch (e) {
      console.error("[Push] Failed to purge expired subscriptions:", e);
    }
  }
